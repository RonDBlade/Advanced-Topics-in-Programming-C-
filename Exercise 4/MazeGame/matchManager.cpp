...
vector<void*> matchManager::registerSoFiles(vector<string>& algoFiles, vector<string>& mazeFiles){
    void *handle;
    vector<void*> fileHandles;
    // We hold the algorithms in a vector of pairs - the algorithm name and it's generating function,
    // upon registration, the algorithms will register itself to the pair.
    ...
    loadMazes(mazeFiles);
    // Release the algorithms from the linking to the .so files.
    instance.loadedAlgorithms.clear();
    return fileHandles;
}
...
void matchManager::runGames(){
    ...
    while((gameID = allGames.popElement(game)) > -1){
        game.runGame();
        // Writing the algorithm moves to a file according to it's name
        if(outputFolder != ""){
...
void matchManager::printScores(){
    // The function prepares all the data for printing in outputData.
...
void matchManager::setOutputFolder(string outputPath){
    outputFolder = outputPath;
}

void matchManager::setNumberOfThreads(int numberOfThreads){
